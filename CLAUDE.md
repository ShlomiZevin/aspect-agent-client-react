...
`ChatWidget`, which binds it to one fresh conversation id via
`SCOPE_STORAGE_KEY` (a later plain conversation in the same iframe can never
inherit a stale scope); `ChatProvider` sends `moduleScope` per turn and
re-learns it from the conversation's server-side metadata stamp on history
reload — the opener's prop only seeds the first turns. Scoped conversations
get a banner (`ChatContainer`), a history tag (both history surfaces), and
their own welcome (`scope.welcome`) instead of the dataset's quick-question
tiles. Module ACTION CARDS arrive as persisted `chat_action` thinking steps
and render through the kind→renderer registry in
`src/components/chat/Message/ChatActionCard.tsx` — one branch per action
`kind`, unknown kinds render nothing; the card syncs its proposal's CURRENT
server state on mount, and executed changes reach the hosting page over the
app's first cross-iframe `postMessage` bridge (`aspect:module-action`,
