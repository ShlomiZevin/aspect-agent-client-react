          ...
          {history.map(run => (
            <>
              <tr key={run.id}>
                ...
                <td>{run.triggered_by === 'index' ? '—' : run.files_loaded != null ? `${run.files_loaded}/${run.total_files ?? '?'}` : '—'}</td>
                <td>{run.triggered_by === 'index' ? '—' : run.total_rows ? Number(run.total_rows).toLocaleString() : '—'}</td>
